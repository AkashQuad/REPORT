...
TWBX_CONTAINER = os.getenv("TWBX_CONTAINER")      # tableau-raw
CSV_CONTAINER = os.getenv("CSV_CONTAINER")        # tableau-datasources
...
def download_twbx_from_blob(folder_name: str) -> str:
    """
    Downloads <folder_name>.twbx from tableau-raw container
    """
    blob_service = BlobServiceClient.from_connection_string(
        AZURE_STORAGE_CONNECTION_STRING
    )
    ...
    twbx_name = f"{folder_name}.twbx"

    try:
        data = container.download_blob(twbx_name).readall()
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".twbx")
        tmp.write(data)
        tmp.close()

        log.info(f"Downloaded TWBX: {twbx_name}")
        return tmp.name

    except Exception as e:
        raise Exception(f"TWBX not found: {twbx_name}") from e
...
def migrate_static(
    ...
    try:
        ...
        for r in relationships:
            valid_tables.add(clean_table_name(r["fromTable"]))
            valid_tables.add(clean_table_name(r["toTable"]))

        # ----------------------------------------------------
        # 3. LOAD CSVs BASED ON METADATA
        # ----------------------------------------------------
        blob_service = BlobServiceClient.from_connection_string(
            AZURE_STORAGE_CONNECTION_STRING
        )
        ...
        for blob in container.list_blobs(name_starts_with=prefix):
            ...
            table_name = clean_table_name(base.split("_")[1])
        ...
        for r in relationships:
            pbi_relationships.append({
                "name": f"{r['fromTable']}_{r['toTable']}",
                "fromTable": clean_table_name(r["fromTable"]),
                "fromColumn": r["fromColumn"],
                "toTable": clean_table_name(r["toTable"]),
                "toColumn": r["toColumn"],
                "crossFilteringBehavior": "BothDirections",
            })

        # ----------------------------------------------------
        # 5. DATASET DEFINITION (IMPORTANT FIX)
        # ----------------------------------------------------
        dataset_payload = {
            "name": f"{REPORT_NAME}_DS",
            "defaultMode": "Push",
        ...
        for table, df in blob_tables.items():
            ...
            log.info(f"Pushed {len(rows)} rows into {table}")
